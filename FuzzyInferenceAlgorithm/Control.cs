...
namespace FuzzyInferenceAlgorithm
{
    public class Control
    {
        ...
        List<Rule> _rules = new List<Rule>(); //правило вывода

        public double CountMax() //метод считает 
        {
        ...
        public List<Rule> GetRules() //получаем список правил вывода
        {
            return _rules;
        }
        ...
        {
            EnterListOfRules(); //задание правил вручную
            GetResultUsingKnowledge(double.Parse(procent), double.Parse(time)); //результат знаний 
        ...
        public void GetResultUsingKnowledge(double procent, double time)
        {
            ...
            //успех 0-100% (100.00)
            double n = 10000; //кол-во точек на графике

            //заполняем массив каждой точки результирующей функции нулями
            for (int i = 0; i < n; i++)
                y.Add(0);
        ...
        public List<Grafic> MakeListGrafsWithOtsech(double procent, double time)
        {
            ...
            //получить результирующий график для правила 2
            //ищем высоту 1 правила
