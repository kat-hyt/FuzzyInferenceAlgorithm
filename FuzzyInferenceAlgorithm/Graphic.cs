...
namespace FuzzyInferenceAlgorithm
{
    public class Grafic
    {
        ...
        /// <summary>
        /// Вычислить значение y(х) для текущего ГРАФИКА. Функцию опеределяется произвольно
        /// </summary>
        /// <param name="x"></param>
        /// <returns></returns>
        /// 
        ...
        public int GetCaseNumber(double y) //возвразщает номер функции
        {
            ...
            if (_functions[0].P1.Y <= y && _functions[0].P2.Y >= y)
            {
                //если 2-ая функция не пересекает (выше отсечения) у треуголника
                if (_functions[1].P1.Y > y && _functions[1].P2.Y > y)
                {
                    return 2;
                }
        ...
        /// <summary>
        /// Метод генерирует результирующий график результата по выбранному правилу
        /// </summary>
        public Grafic MakeOtsech(double y)
        {
            //Этапы алгоритма по модели Мондани:
            //1. СОЗДАТЬ спиок для хранения точек нового ГРАФИКА.
            ...
            //3. ДОБАВИТЬ в список точек значения точек для функций с учетом отсечений.
            //4. СФОРМИРОВАТЬ новые функции по списку точек.
            ...
            //отображение всех функций на графике
            ...
            //отображение нового графика, учитывая, что функции отсеклись
